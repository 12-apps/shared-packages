 ...
 * so every one of them is written as a length string — otherwise a `p={2}` box
 * renders 128px of padding on the web and 16dp on native, which is the one
 * thing this component exists to rule out.
 *
 * Spacing, radius and the border are already the host theme's own px (read
 * through `uiThemeFromMui`), so they stay px: through `rem` they would scale
 * twice. `width`/`height` are the caller's design px, so they go through `rem`.
 */
const PX_KEYS = new Set<keyof BoxLayout>([
  'paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft',
  'marginTop', 'marginRight', 'marginBottom', 'marginLeft',
  'gap', 'borderRadius', 'borderWidth',
]);
const DESIGN_PX_KEYS = new Set<keyof BoxLayout>(['width', 'height']);

function lengthOf(theme: Theme, key: keyof BoxLayout, value: number): string | number {
  if (DESIGN_PX_KEYS.has(key)) return rem(theme, value);
  return PX_KEYS.has(key) ? `${value}px` : value;
}

export function layoutToSx(layout: BoxLayout, theme: Theme): Record<string, string | number> {
  return Object.fromEntries(
    Object.entries(layout).map(([key, value]) => [
      key,
      typeof value === 'number' ? lengthOf(theme, key as keyof BoxLayout, value) : value,
    ]),
  );
}
...
export const Box = React.forwardRef<HTMLDivElement, BoxProps>(
  ({ children, sx, ...props }, ref) => {
    ...
    const resolved = layoutToSx(resolveBoxLayout(layout, theme), muiTheme) as SxProps<Theme>;
