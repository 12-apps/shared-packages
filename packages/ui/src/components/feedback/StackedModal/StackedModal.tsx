...
export const ModalStackProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const value = useMemo(
    () => ({
      ...
      isModalInStack,
      getModalRole: _getModalRole,
    }),
    [stack, pushModal, popModal, clearStack, isModalInStack, _getModalRole],
  );
  ...
  ({ theme, modalRole, isAnimating, glass, rtl, customMaxWidth }) => {
    ...
    return ({
      ...
      '& .MuiDialog-paper': {
        ...
        ...(modalRole === 'primary' && {
          // The top panel is viewport-aware and narrows as the screen grows: full
          // width on mobile, 80vw on tablet/small desktop, 60vw on regular desktops
          // (lg up — including ~1080p/1440p/1080-wide), and 40vw only on ultra-wide
          // screens (≥2200px) so the receding panel stays visible on huge monitors.
          // A `maxWidth` prop, when given, caps it further. Panels below expand to
          // full width (the `secondary` role), producing the GTM stacking effect.
          ...
          [theme.breakpoints.up('lg')]: {
            width: maxWidthPx ? `min(60vw, ${maxWidthPx}px)` : '60vw',
            maxWidth: maxWidthPx ? `${maxWidthPx}px` : '60vw',
          },
          '@media (min-width:2200px)': {
            width: maxWidthPx ? `min(40vw, ${maxWidthPx}px)` : '40vw',
            maxWidth: maxWidthPx ? `${maxWidthPx}px` : '40vw',
          },
        }),
        ...(modalRole === 'secondary' && {
