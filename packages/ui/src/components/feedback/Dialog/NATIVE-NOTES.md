...
- `borderRadius` has no effect on `variant="drawer"` on EITHER renderer: the web builds `${radius}px 0 0 ${radius}px` out of `theme.spacing()`, which is already a px string, so the declaration is invalid and MUI's own `shape.borderRadius` is what paints. Native paints the same radius so the two agree; the fix is a web change (`Dialog.styles.ts`), made once, for both.
