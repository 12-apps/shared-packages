...
- **Typed suggestions**: Optional `search`/`link` items — a `link` shows a link icon and opens its URL in a new tab; a `search` shows a search icon and fills the input.
- **Inline completion**: Optional ghost remainder and `Tab/→` acceptance (skips `link` items).
...
export interface AutocompleteProps<T = any> {
  ...
  /** Resolve a suggestion's type: 'link' opens getUrl(item) on select; 'search' fills the input. */
  getSuggestionType?: (item: T) => 'search' | 'link' | undefined;
  /** Resolve the URL for a 'link' suggestion. */
  getUrl?: (item: T) => string | undefined;
  /** Open a 'link' suggestion. Default: window.open(url, '_blank', 'noopener,noreferrer'). */
...
### Typed suggestions (search vs. link)

Provide `getSuggestionType` (and `getUrl` for links) to render a per-row icon and
change select behavior. A `link` suggestion opens its URL via `openLink` (default
`window.open(url, '_blank', 'noopener,noreferrer')`) and does **not** change the
input value or add a chip; a `search` suggestion behaves as normal. Untyped items
(no `getSuggestionType`) keep the classic icon-less behavior. Ghost-text inline
completion only considers `search`/untyped items.

```tsx
<Autocomplete<QuickSearchItem>
  value={q}
  onChange={setQ}
  suggestions={items} // [{ id, text, type: 'search' | 'link', url? }]
  getKey={(i) => i.id}
  getLabel={(i) => i.text}
  getSuggestionType={(i) => i.type}
  getUrl={(i) => i.url}
  // openLink={(url) => router.push(url)} // optional: route instead of new tab
/>
```
