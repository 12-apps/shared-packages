...
export const Sheet: React.FC<SheetProps> = ({
  ...
  const getSizeStyles = () => {
    ...
    // Horizontal (left/right) sheets are viewport-aware: a px floor keeps the
    // panel usable, `…vw` lets the larger presets grow on wide screens (lg ≥32%,
    // xl ≥40% of the viewport), and the outer `min(92vw, …)` guarantees the
    // panel never overflows a small screen. Vertical presets stay fixed-height.
    switch (size) {
      case 'xs':
        return isHorizontal ? { width: 'min(92vw, 240px)' } : { height: 200 };
      case 'sm':
        return isHorizontal ? { width: 'min(92vw, 320px)' } : { height: 300 };
      case 'md':
        return isHorizontal ? { width: 'min(92vw, 400px)' } : { height: 400 };
      case 'lg':
        return isHorizontal ? { width: 'min(92vw, max(560px, 32vw))' } : { height: 500 };
      case 'xl':
        return isHorizontal ? { width: 'min(92vw, max(720px, 40vw))' } : { height: 600 };
      case 'full':
        ...
        <SwipeableDrawer
          ...
          PaperProps={{
            ...
            sx: {
              ...
              ...style,
              overflow: 'visible',
              // Expand only the cross axis so the chosen `size` still governs
              // the main axis: side sheets keep their (responsive) width and go
              // full-height; top/bottom sheets keep their height and go full-width.
              ...(fullHeight &&
                !isDraggableVariant &&
