...
- Hover, active and focus-within (the lift, the hue shadow, the icon tilt, the focus ring) have no touch equivalent; the close button's pressed state stands in for its hover.
- The hairline the web draws around a `Button` placed inside the message is a descendant selector; native children are rendered as given.
