...
export interface TabsProps extends Omit<MuiTabsProps, 'variant' | 'children' | 'indicatorColor' | 'scrollButtons'> {
  ...
  /**
   * Pin the tab list in place (CSS `position: sticky`) while the tab panels
   * scroll beneath it. Useful for long, scrollable pages.
   */
  sticky?: boolean;

  /**
   * Offset from the top of the viewport (in px) when `sticky` is enabled — set
   * this to the height of any fixed header so the tab list parks just below it.
   * Defaults to `0`.
   */
  stickyOffset?: number;
