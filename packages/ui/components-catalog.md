...
### SectionOnboarding
**Purpose**: Render a configurable page section three ways by lifecycle so first-time users get a friendly hero + CTA instead of a wall of empty fields
**Use Cases**: Settings/integration sections (payments, AI, location), any "set up once then manage" surface
**Features**: `unconfigured` hero / `in-progress` steps + form / `configured` compact summary with reveal toggle, composes EmptyState + Stepper, server-derivable status, aria-expanded toggle
**Location**: `src/components/data-display/SectionOnboarding`
**Docs**: `src/components/data-display/SectionOnboarding/SectionOnboarding.md`
...
### CreatableSelect
**Purpose**: Single-select searchable combobox that can create a new option inline
**Use Cases**: Pick-or-create pickers (category, tag, brand, supplier), searchable selects with a growing option list
**Features**: Type-to-filter, create-on-Enter via `onCreate`, controlled value, loading/disabled states, MUI-themed (matches other form fields)
**Location**: `src/components/form/CreatableSelect`
**Docs**: `src/components/form/CreatableSelect/CreatableSelect.md`
...
**Use Cases**: List/content pages (Favorites, Personal Space, Recents, Collections), any page needing selection chrome + sort/filter controls
**Features**: Selection cluster (Select All/Clear All/count/actions slot), rightControls slot, ViewSelector (grid/list + zoom), SortByDropdown (Order/Sort sections), MultiSelectDropdown (checkbox + counts + Clear), FilterTrigger (funnel + active dot)
**Location**: `src/components/layout/ContentToolbar`
**Docs**: `src/components/layout/ContentToolbar/ContentToolbar.md`
...
### TableFilter
**Purpose**: Compound filter-panel that slides in beside table content
**Use Cases**: List/table pages needing a keyword + faceted filter panel (paired with ContentToolbar's FilterTrigger)
**Features**: Provider + Layout/Main/Panel (animated 280px aside), Keyword search, Section groups, CheckboxField (multi-select + counts), RangeField (min/max), Clear All Filters
**Location**: `src/components/layout/TableFilter`
**Docs**: `src/components/layout/TableFilter/TableFilter.md`
