 ...
 *
 * Pure, framework-agnostic validators — NO zod, NO react-hook-form. Each
 * validator is curried and returns a `string` error message for invalid input
 * or `undefined` when the value is valid.
 *
 * @module forms-core/validators
 */
...
/**
 * The first error message a field's validators produce, or `undefined` when
 * every validator passes. Short-circuits on the first failure.
 */
const firstFieldError = (validators: Validator[], value: string): string | undefined => {
...
export const validateFields = <T extends Record<string, string>>(
  ...
  for (const field of Object.keys(schema) as (keyof T)[]) {
    ...
    const error = firstFieldError(validators, values[field] ?? '');
    if (error !== undefined) {
      errors[field] = error;
    }
  }
