...
export class ActionError extends Error {
  ...
  /**
   * Per-field messages when a single failure attributes to MORE than one field
   * (e.g. several duplicate SKUs across a product's variation rows). Takes
   * precedence over {@link field}; when set, {@link createServerAction} passes
   * this map straight through as `fieldErrors`.
   */
  ...
  return async (input: I): Promise<Result<O>> => {
    ...
    try {
      ...
      if (error instanceof ActionError) {
        // A multi-field map (fieldErrors) wins over the single-field shorthand so
        // one failure can highlight several inputs (e.g. duplicate variation SKUs).
        const fieldErrors =
          error.fieldErrors ?? (error.field ? { [error.field]: error.message } : undefined);
