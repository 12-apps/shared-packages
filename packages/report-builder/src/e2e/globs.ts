...
/**
 * Where this package's report journeys and their steps live, ON DISK, in the
 * consumer's `node_modules` (FUT-755).
 *
 * `defineBddConfig` takes filesystem globs, not module specifiers — it hands
 * them to a glob matcher, so `'@12-apps/report-builder/features/**'` matches
 ...
 * A host that hard-codes `node_modules/@12-apps/report-builder/...` is broken by
 ...
 * three. This is `@12-apps/payments-e2e`'s arrangement, for the same reasons.
 */
...
function packageRoot(): string {
  return dirname(require_.resolve('@12-apps/report-builder/package.json'));
}

/** Every packaged `.feature`, for `defineBddConfig({ features })`. */
export const reportsFeatures: string = join(packageRoot(), 'features/**/*.feature');
 ...
 * feature living in `node_modules/@12-apps/report-builder/features/x.feature`
 * compiles to `.features-gen/node_modules/@12-apps/report-builder/features/
 * x.feature.spec.js` — a path Playwright then IGNORES, because its default
 * `testIgnore` excludes `**\/node_modules/**`.
 *
 * The result is the worst kind of green: bddgen reports six features compiled,
 * Playwright collects zero specs from them, and the run passes with the whole
 * packaged suite silently absent.
 *
 * A host running TWO packaged suites gives `featuresRoot` the directory both
 * sit under, since bddgen takes exactly one — and that is safe to do, because a
 * feature outside it is a hard `exit` from bddgen ("All feature files should be
 * located underneath featuresRoot"), never a quiet omission.
 */
export const reportsFeaturesRoot: string = join(packageRoot(), 'features');
 ...
 * `node_modules` by design. Shipping `src/e2e/**\/*.ts` here would produce a
 * subpath that type-checks, publishes, installs, and then throws on the
 * consumer's first test run.
 *
 * The host's own steps glob must ALSO be listed — that is where its
 * `defineReportsWorld` call lives, and playwright-bdd imports every step file
 * before any scenario runs, which is what makes the registration land in time
 * in every worker.
 */
export const reportsSteps: string = join(packageRoot(), 'dist/e2e/steps/**/*.js');
