 ...
 * them to a glob matcher, so `'@12-apps/rbac/features/**'` matches
 ...
 * host that hard-codes `node_modules/@12-apps/rbac/...` is broken by
...
function packageRoot(): string {
  return dirname(require_.resolve('@12-apps/rbac/package.json'));
}

/** Every packaged `.feature`, for `defineBddConfig({ features })`. */
export const rbacFeatures: string = join(packageRoot(), 'features/**/*.feature');
 ...
 * omission.
 */
export const rbacFeaturesRoot: string = join(packageRoot(), 'features');
 ...
 * `defineRbacWorld` call lives, and playwright-bdd imports every step
 * file before any scenario runs, which is what makes the registration land in
 * time in every worker.
 */
export const rbacSteps: string = join(packageRoot(), 'dist/e2e/steps/**/*.js');
