...
/**
 * The Auth.js entrypoint (FUT-665) — the RUNTIME-BOUND half of this package.
 *
 * It lives under `./server` for the reason report-builder's runtime-bound half
 * does: the package root must stay importable without dragging a runtime in.
 * This file value-imports `@auth/core` and applies environment defaults as a
 * module side effect, so a job that wants `hashToken` and nothing else would
 * have paid for Auth.js just by importing the root.
 *
 ...
 * reproduced here against `@auth/core` (which is what the wrapper called); the
 * third becomes an `auth(request)` that takes the request explicitly — which is
 * what its one caller had in hand anyway, and is why nothing about the sign-in
 * flow, the cookie or the callback URLs changed.
 */
