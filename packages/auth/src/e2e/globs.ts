 ...
 * them to a glob matcher, so `'@12-apps/auth/features/**'` matches nothing
 ...
 * host that hard-codes `node_modules/@12-apps/auth/...` is broken by pnpm's
...
function packageRoot(): string {
  return dirname(require_.resolve('@12-apps/auth/package.json'));
}
 ...
 * feature living in `node_modules/@12-apps/auth/features/x.feature`
 * compiles to `.features-gen/node_modules/@12-apps/auth/features/
 ...
 * all while every OTHER entry point ships raw `.ts` through its `exports` map.
 ...
 * in every worker.
 */
export const authSteps: string = join(packageRoot(), 'dist/e2e/steps/**/*.js');
