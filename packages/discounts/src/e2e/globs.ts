 ...
 * them to a glob matcher, so `'@12-apps/discounts/features/**'` matches
 ...
 * host that hard-codes `node_modules/@12-apps/discounts/...` is broken by
...
function packageRoot(): string {
  return dirname(require_.resolve('@12-apps/discounts/package.json'));
}

/** Every packaged `.feature`, for `defineBddConfig({ features })`. */
export const discountsFeatures: string = join(packageRoot(), 'features/**/*.feature');
 ...
 * omission.
 */
export const discountsFeaturesRoot: string = join(packageRoot(), 'features');
 ...
 * `defineDiscountsWorld` call lives, and playwright-bdd imports every step
 * file before any scenario runs, which is what makes the registration land in
 * time in every worker.
 */
export const discountsSteps: string = join(packageRoot(), 'dist/e2e/steps/**/*.js');
