 ...
 * `app-shell` shipped without one, and by the time anybody looked it had
 * accumulated five docstrings naming the application it was extracted from, and
 * — the part that actually reached users — two `DEFAULT_*` message tables
 * holding that application's pt-BR copy, spread UNDER a host's override so a
 * partial one silently inherited the rest.
 *
 * Both were removed by hand. This file is what stops the next one, and it is
...
describe('the shipped source names no adopter', () => {
  it('finds files to scan, so an empty pass cannot be a broken walk', () => {
    ...
    expect(files).toContain('react/messages.ts');
    expect(files).toContain('server/config.ts');
  });
...
describe('the shipped source ships nobody else\'s words', () => {
  it('exports no DEFAULT_ copy table for a host to inherit by accident', () => {
    ...
    //
    // Scoped to copy rather than to every `DEFAULT_`: `DEFAULT_SURFACE` and
    // `DEFAULT_THEME_TOKENS` are this package's own machinery (a colour ramp is
    // not somebody's language), and forbidding them outright would be a rule
    // nobody could keep.
    ...
    expect(exported.filter((name) => /^DEFAULT_.*(LABELS?|COPY)$/.test(name))).toEqual([]);
  });
});
