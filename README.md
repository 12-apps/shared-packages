...
| `@12-apps/i18n` | The locale axis for packages whose copy is already the host's: the canonical locale list, locale packs and the resolver a config field takes, `Accept-Language` negotiation with a stated precedence, locale-aware money/number/date formatting and parsing, a React provider and the cross-locale parity assertion. Zero dependencies, and nothing here imports it — a package ships a plain record keyed by tag. |
| `@12-apps/impersonation` | Impersonation / preview sessions: an operator previews as a tenant user, reads are scoped, writes are refused unless opted in, money paths are always refused, and revoking the entitlement ends the session live. The cookie, the write gate, the routes, the banner, the start dialog and the Gherkin journeys are the package's; the path tables, the directory, the trail and every sentence are the host's and arrive as config. |
