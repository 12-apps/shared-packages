...
describe("the block templates the picker offers", () => {
  ...
  it("keeps a host group whose templates compile against the host catalog", () => {
    ...
    const runnable = groups
      .flatMap((group) => group.templates)
      .filter((one) => one.spec !== null);
  ...
  it("always offers the blank escape hatch, last, even with no host groups", () => {
    const last = blockTemplateGroups([], PT_BR_BLANK_BLOCK_TEMPLATE_COPY).at(
      -1,
    );
    expect(last?.templates).toEqual([
      blankBlockTemplate(PT_BR_BLANK_BLOCK_TEMPLATE_COPY),
    ]);
    expect(blankBlockTemplate(PT_BR_BLANK_BLOCK_TEMPLATE_COPY).spec).toBeNull();
  });
});
...
describe("top-N folds the remainder rather than dropping it", () => {
  it('returns the leaders plus "Outros", and the total still balances', async () => {
    ...
    const shown = result.rows.reduce(
      (total, row) => total + Number(row.sum_totalCents),
      0,
    );
...
describe("the chart spec the published package emits", () => {
  it("carries no axis title and never smooths the line", async () => {
    ...
    if (result.render.kind !== "chart")
      throw new Error("expected a chart render");
        ...
        filters: [
          {
            field: "method",
            operator: "eq",
            value: defaultValueFor(catalog.entities.orders.fields.method),
          },
        ],
      },
      options,
...
describe("a draft knows whether it has unsaved changes", () => {
    ...
    blocks: [
      {
        id: "b1",
        span: 6,
        spec: { entity: "orders", measures: [{ field: "totalCents" }] },
      },
      {
        id: "b2",
        span: 6,
        spec: { entity: "payments", measures: [{ field: "amountCents" }] },
      },
    ],
  };
  ...
  it("is clean when a block is dropped back where it started", () => {
    expect(
      isDirty(saved, { ...saved, blocks: [saved.blocks[0], saved.blocks[1]] }),
    ).toBe(false);
  });

  it("is clean when the keys arrive in a different order", () => {
    // The trap JSON.stringify comparison falls into.
    expect(isDirty(saved, { blocks: saved.blocks, name: saved.name })).toBe(
      false,
    );
  });
  ...
  it("is dirty when a block genuinely moves", () => {
    expect(
      isDirty(saved, { ...saved, blocks: [saved.blocks[1], saved.blocks[0]] }),
    ).toBe(true);
  });
