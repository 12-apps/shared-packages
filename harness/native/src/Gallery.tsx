...
function ButtonSection({ onCount, presses }: { onCount: () => void; presses: number }): React.JSX.Element {
  return (
    <Section title="Button" testID="section-button">
      ...
      <Stack direction="row" gap={1} wrap align="center">
        ...
        {/* Wired to the counter on purpose: a disabled button that fired would be visible. */}
        <Button disabled onClick={onCount} testID="button-disabled">
          Indisponível
        </Button>
      ...
      <Stack direction="row" gap={1} align="center">
        <Button onClick={onCount} testID="button-counter">
          Contar
        </Button>
...
export function Gallery(): React.JSX.Element {
  ...
  const onCount = React.useCallback(() => setPresses((n) => n + 1), []);
